...
## 2. 用户与角色
- 用户首次进入小程序时，系统自动分配随机昵称与默认头像，用户可在个人中心或菜单选择页的编辑入口自行修改
- 微信用户可以访问多个菜单。
...
- 当用户在任何菜单中都没有角色时，菜单列表显示空状态提示“暂无菜单，点击‘新建菜单’或通过邀请加入”，同时禁用“进入菜单”按钮并提示用户先创建或加入菜单
...
- 菜品编辑页面：维护菜品名称、描述、主图、价格、标签、上下架状态、所属分类。菜品仅能归属一个分类。菜品可关联自定义选项并同步更新。页面底部有两个按钮"删除菜品","保存菜品"。
...
- 已下架的菜品和可下单的菜品分成两个区域展示，可下单的菜品在上方列出，有分隔栏文字"可下单"。已下架的菜品在下方列出，分隔栏文字为"已下架"，且菜品卡片为渲染为灰色，意在提示用户不可用。
...
#### 3.5.1 订单
- 可以查看顾客提交的点菜订单，订单列表中的列表项展示编号、状态、时间、总价，整卡可点击进入订单详情；无需额外“查看详情”按钮。
- 列表项内保留订单操作按钮：当订单状态为已下单时展示“取消”“接单”，当订单状态为处理中时展示“取消”“完成”，点击对应按钮即可更新订单状态。
- 订单详情里展示了包含编号、状态（已下单→处理中→已完成/已取消）、时间、总价、菜品项（含选项快照）、顾客备注。按照提交时间倒序排列。点击菜品项可以进入菜品的详情页面。

#### 3.5.2 通知
- 通知页面位于 tabbar “点菜通知”页签，展示用户所有通知，并通过懒加载的方式，仅在用户即将看到时再查询展示下一页的通知。
- 厨师操作导致订单状态改变时，发送通知触达订单的创建顾客。
- 点击通知能跳转到相关订单详情页。
- 通过10秒轮询一次来查询自己的消息通知，并在 tabbar 的通知栏上展示未读通知数量。
...
- **分类**：关联菜单，包含名称、排序权重。
- **菜品**：关联分类和自定义选项集合，包含描述、价格、标签等。可关联菜谱（`recipeId`，选填）。
